...
describe('formatDate', () => {
  it('formats a valid date string correctly', () => {
    ...
    expect(formatDate('2022-07-04')).toBe('Jul 4, 2022')
  })

  it('handles single digit days correctly', () => {
    expect(formatDate('2024-01-01')).toBe('Jan 1, 2024')
    expect(formatDate('2024-02-09')).toBe('Feb 9, 2024')
  })
  ...
  it('returns "Invalid Date" for invalid date strings', () => {
    ...
    expect(formatDate('2024-02-30')).toBe('Mar 1, 2024') // JS Date rolls over
    expect(formatDate('not a date')).toBe('Invalid Date')
  })

  it('handles leap year correctly', () => {
    expect(formatDate('2024-02-29')).toBe('Feb 29, 2024') // Valid leap year
    expect(formatDate('2023-02-29')).toBe('Mar 1, 2023') // Invalid leap year rolls over
  })
