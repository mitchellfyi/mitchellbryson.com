...
export function RocketIcon(props) {
  return (
    <BaseIcon strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" {...props}>
...
export function getProjectIcon(project) {
  ...
  if (name.includes('phone') || name.includes('validator')) return PhoneCheckIcon
  return CodeIcon
}

export function getProjectColor(project) {
  const name = (project?.name || project?.title || '').toLowerCase()
  if (name.includes('launchonomy')) return 'text-indigo-600 dark:text-indigo-400'
  if (name.includes('pitch')) return 'text-pink-600 dark:text-pink-400'
  if (name.includes('extension')) return 'text-emerald-600 dark:text-emerald-400'
  if (name.includes('inbox') || name.includes('triage')) return 'text-amber-600 dark:text-amber-400'
  if (name.includes('commerce') || name.includes('protocol')) return 'text-teal-600 dark:text-teal-400'
  if (name.includes('phone') || name.includes('validator')) return 'text-rose-600 dark:text-rose-400'
